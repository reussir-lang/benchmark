(* Std-collection workload on OCaml's standard ordered map
   ...
   threaded linearly through every round and nothing is retained, so a
   persistent tree can drop or reuse replaced nodes immediately. Final
   size 400,944. *)
