(* Std-collection workload on OCaml's standard hash map
   ...
   373,649; each ring slot ends holding a ~373k-entry version. The
...
let ops_n = 2_000_000
let expected = 326_234_472_953_519L
...
let () =
  ...
  for i = 0 to ops_n - 1 do
    ...
    x := lcg !x;
    if Int64.rem !x 512L = 0L then
      let slot = Int64.to_int (Int64.rem (Int64.div !x 512L) 8L) in
