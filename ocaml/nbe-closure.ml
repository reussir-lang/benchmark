...
type value =
  | VVar of int
  | VApp of value * value
  | VLam of int * term * env

and env = Cons of int * value * env | Nil

let rec max_of_names names acc =
  match names with id :: rest -> max_of_names rest (max acc id) | [] -> acc
...
and vapp v1 v2 =
  match v1 with
  | VLam (id, body, env) -> eval (Cons (id, v2, env)) body
  | _ -> VApp (v1, v2)
...
let rec quote names = function
  ...
  | VLam (id, body, env) ->
      ...
      Lam (y, quote (y :: names) (eval (Cons (id, VVar y, env)) body))
...
let () =
  let rec rounds i acc = if i = 7 then acc else rounds (i + 1) (acc + nbe_test i) in
  let result = rounds 0 0 in
  if result <> 7_000_021 then (
    Printf.eprintf "FAIL: expected 7000021, got %d\n" result;
    exit 1)
