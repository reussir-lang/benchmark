...
fn eval(env: Rc<Env>, x: &Term) -> Value {
    match x {
        ...
        Term::Lam(x_id, t) => Value::VLam(*x_id, t.clone(), env),
        Term::Let(x_id, t, u) => {
...
fn vapp(v1: Value, v2: Value) -> Value {
    match v1 {
        Value::VLam(x_id, body, env) =>
            eval(Rc::new(Env::Cons(x_id, v2, env)), body.as_ref()),
...
fn quote(n: &Names, v: &Value) -> Term {
    match v {
        ...
        Value::VLam(x_id, body, env) => {
            ...
            let closure_env = Rc::new(Env::Cons(*x_id, Value::VVar(y), env.clone()));
            Term::Lam(y, Rc::new(quote(&n2, &eval(closure_env, body.as_ref()))))
        }
    }
}
