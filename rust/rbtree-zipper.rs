...
fn ins(t: &Tree, k: i64, v: bool, z: Rc<Zipper>) -> Tree {
    match t {
        Tree::Branch(c, l, kx, vx, r) => {
            if k < *kx {
                ins(l, k, v, rc_z(Zipper::NodeL(*c, z, *kx, *vx, r.clone())))
            } else if k > *kx {
                ins(r, k, v, rc_z(Zipper::NodeR(*c, l.clone(), *kx, *vx, z)))
            } else {
...
fn main() {
    ...
    let p = fold_test(n);
    if p != 1_000_000 {
        eprintln!("FAIL: expected 1000000, got {}", p);
        std::process::exit(1);
    }
}
