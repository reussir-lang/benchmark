...
// workload. Same Zipfian mixed-op workload and checksum as
// hash-map-linear.rs; with no retained version every `_mut` update
// sees a unique handle and reuses nodes in place.
...
fn main() {
    ...
    let result =
        acc + m.iter().map(|(k, v)| k * 31 + v).sum::<i64>() + 7 * (m.size() as i64);
    if result != EXPECTED {
